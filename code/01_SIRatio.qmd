...
Finally, we tested if females showed evidence of stress effects in the raw time in the interaction zone, that may be masked when computing the SI ratio. Males show reduced time in ther interaction zone only during the Target trial. In contrast, Females show a modest increase in time in the interaction zone in the No Target trial, and a modest decrease during the Target trial compared to controls. 
...
m_aov <- aov(IZ_time ~ Condition * Trial_type + cohort + Error(id_full / Trial_type), m_data)
f_data <- SIT_raw[SIT_raw$Sex == 'Female',]
f_aov <- aov(IZ_time ~ Condition * Trial_type + cohort + Error(id_full / Trial_type), f_data)
...
pander(m_aov, caption = 'rmANOVA in Males')
kable(rbind(sidak_cond_tt_m[1],
            sidak_cond_tt_m[2],
            sidak_tt_cond_m[1],
            sidak_tt_cond_m[2]), digits = 2, align='c', caption = 'Sidak Post-Hoc Planned Contrasts in Males')

pander(f_aov, caption = 'rmANOVA in Females')
kable(rbind(sidak_cond_tt_f[1],
            sidak_cond_tt_f[2],
            sidak_tt_cond_f[1],
            sidak_tt_cond_f[2]), digits = 2, align='c', caption = 'Sidak Post-Hoc Planned Contrasts in Females')
