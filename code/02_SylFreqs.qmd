...
SIT_z <- readRDS(glue('{data_path}/SIT_z_diff_primary.rds'))
...
hist_m <- ggplot(SIT_z[SIT_z$Sex == 'Male',], 
              ...
              geom_density_ridges(aes(linetype = Trial_type), 
                                  alpha = 0.1, size = 1) + 
              scale_x_continuous(limits = c(-5, 5), breaks = c(-5,0,5), name = 'Frequency z-Score') + 
              ylab('Syllable') + 
...
```{r}
#| label: fig 4abcd model + contrasts 
#| echo: false
